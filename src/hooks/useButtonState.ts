...
export const useMultiButtonState = (configs: ButtonStateConfig[]) => {
  // Create button states array using individual hooks
  const buttonStates: ButtonStateReturn[] = [];
  for (let i = 0; i < configs.length; i++) {
    // eslint-disable-next-line react-hooks/rules-of-hooks
    buttonStates.push(useButtonState(configs[i]!));
  }
  ...
  // 为每个按钮添加全局loading检查
  const enhancedStates = buttonStates.map((buttonState, _index) => ({
    ...buttonState,
